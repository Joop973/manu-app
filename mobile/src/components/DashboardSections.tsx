...
/**
 * F-005: Dashboard-Sektionen als ID-basierte Karten, die über `dashboardOrder`
 * sortiert und über `dashboardHidden` ausblendbar sind.
 */

export const SECTION_META: Record<string, { label: string; icon: string }> = {
  header:        { label: 'Kopf',                 icon: '🏛' },
  clipboard:     { label: 'Zwischenablage',       icon: '📋' },
  balance:       { label: 'Bilanz',               icon: '💰' },
  leftover:      { label: 'Heute verfügbar',      icon: '☀️' },
  monthSlider:   { label: 'Monats-Slider',        icon: '📅' },
  heatmap:       { label: 'Ausgaben-Heatmap',     icon: '🔥' },
  oracle:        { label: 'Orakel',               icon: '👁' },
  goals:         { label: 'Sparziele',            icon: '🎯' },
  quickActions:  { label: 'Schnellaktionen',      icon: '⚡' },
  achievements:  { label: 'Achievements',         icon: '🏆' },
  properties:    { label: 'Imperien',             icon: '🏛' },
};
...
export function renderSection(id: string) {
  switch (id) {
    case 'header': return <SectionHeader key={id} />;
    case 'clipboard': return <SectionClipboard key={id} />;
    case 'balance': return <SectionBalance key={id} />;
    case 'leftover': return <SectionLeftover key={id} />;
    case 'monthSlider': return <SectionMonthSlider key={id} />;
    case 'heatmap': return <SectionHeatmap key={id} />;
    case 'oracle': return <SectionOracle key={id} />;
    case 'goals': return <SectionGoals key={id} />;
    case 'quickActions': return <SectionQuickActions key={id} />;
    case 'achievements': return <SectionAchievements key={id} />;
    case 'properties': return <SectionProperties key={id} />;
    default: return null;
  }
}
  ...
  return (
    <View>
      <View style={styles.headRow}>
        ...
        <Text style={text.imperialHeadline}>Manu</Text>
...
function SectionClipboard() {
  ...
  if (!clipboardHint) return null;
  return (
    <View style={styles.clipboard}>
      <View style={{ flex: 1 }}>
        <Text style={[text.caption, { color: palette.imperialGold }]}>ZWISCHENABLAGE</Text>
        <Text style={text.bodyBold}>
          {clipboardHint.amount !== undefined ? formatEuro(clipboardHint.amount) : clipboardHint.iban}{' '}
...
function SectionLeftover() {
  const bookings = useAppStore((s) => s.bookings);
  const templates = useAppStore((s) => s.templates);
  const subscriptions = useAppStore((s) => s.subscriptions);
  const leftover = computeLeftover({ bookings, templates, subscriptions, today: new Date() });
  return (
    <View style={[styles.leftoverCard, shadows.card]}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
        ...
        <HelpHint
          ...
          body="Manu rechnet: aktueller Saldo + erwartete Einnahmen − geplante Pflichten, geteilt durch Tage bis Monatsende."
        />
      </View>
      <Text
        style={[
...
function SectionMonthSlider() {
  const currentMonth = useAppStore((s) => s.currentMonth);
  const setCurrentMonth = useAppStore((s) => s.setCurrentMonth);
  return <MonthSlider value={currentMonth} onChange={setCurrentMonth} />;
}

function SectionHeatmap() {
  const bookings = useAppStore((s) => s.bookings);
  const currentMonth = useAppStore((s) => s.currentMonth);
  const heatmap = buildMonthHeatmap(bookings, currentMonth);
  return (
...
function SectionOracle() {
  ...
  if (oracleTips.length === 0) return null;
  return (
    <View style={{ gap: spacing.md }}>
      ...
      {oracleTips.map((t) => <OracleCard key={t.id} tip={t} />)}
    </View>
  );
}
        ...
        return (
          <View key={g.id} style={[styles.card, shadows.card]}>
            <View style={styles.rowSplit}>
              ...
              <Text style={text.bodyBold}>{formatEuro(g.saved)} / {formatEuro(g.target)}</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  iconBtn: {
    ...
    backgroundColor: 'rgba(212,175,55,0.10)', borderWidth: 1, borderColor: palette.cardBorder,
  },
  balanceCard: {
    backgroundColor: palette.royalBlue, borderRadius: radii.lg, padding: spacing.lg,
    borderWidth: 1, borderColor: palette.cardBorder,
  },
  leftoverCard: {
    backgroundColor: 'rgba(212,175,55,0.08)', borderRadius: radii.lg, padding: spacing.lg,
    borderWidth: 1, borderColor: palette.imperialGold, alignItems: 'center',
  },
  balanceRow: { flexDirection: 'row', marginTop: spacing.lg, gap: spacing.lg },
  card: {
    backgroundColor: palette.royalBlue, borderRadius: radii.lg, padding: spacing.lg,
    borderWidth: 1, borderColor: palette.cardBorder, gap: spacing.sm,
  },
  ...
  clipboard: {
    ...
    backgroundColor: 'rgba(212,175,55,0.10)', borderColor: palette.cardBorder,
